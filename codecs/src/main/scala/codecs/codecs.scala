...
trait EncoderInstances {
  ...
  implicit val boolEncode: Encoder[Boolean] =
...
trait DecoderInstances {
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      case Json.Arr(as) => {
        val decoded = as.map(decoder.decode)
        if (decoded.exists(dc => dc.isEmpty))
          Option.empty
        else
          Option(decoded.map(o => o.get))
      }
      case _ => Option.empty
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person](per => Person(per._1, per._2))

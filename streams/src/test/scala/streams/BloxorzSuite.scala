...
class BloxorzSuite {
  ...
  @Test def `neighborsWithHistory ( million  points )`: Unit =
    new Level1 {
      val expected_val = Set(
        (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up)))

      assertEquals(expected_val, neighborsWithHistory(Block(Pos(1, 1), Pos(1, 1)), List(Left, Up)).toSet)
    }

  @Test def `newNeighborsOnly ( gazillion  points )`: Unit =
    new Level1 {
      val expected_val = Set(
        (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
      ).to(LazyList)

      assertEquals(expected_val, newNeighborsOnly(
        Set(
          (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
          (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
        ).to(LazyList),

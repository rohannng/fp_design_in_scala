...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block,
                           history: List[Move]): LazyList[(Block, List[Move])] = {
    b.legalNeighbors
      .foldRight(LazyList[(Block, List[Move])]())(
        (b, l) => {
          ((b._1, b._2 :: history)) #:: l
        }
      )
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    initial match {
      ...
      case (b, m) #:: tail => {
        // Generate all paths from current block to its valid neighbours
        val current = newNeighborsOnly(neighborsWithHistory(b, m), explored + b)
        // Add all paths from current block , then generate futher paths + also handle remaining in the initial
        current ++ from(tail ++ current, explored + b)
      }
    }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = {
    from(neighborsWithHistory(startBlock, List.empty), Set(startBlock))
  }
  ...
  lazy val solution: List[Move] = {
    pathsToGoal match {
      ...
      case (b, m) #:: tail => {
        // reverse list to get the first move from start
        m.reverse
      }
    }
  }
